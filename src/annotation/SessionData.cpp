...
bool SessionData::load_trace(const fs::path& csv) {
    ...
    // ORIENTATION GUARD. A reference trace is only drawn if it actually agrees with this
    // session's own vertical (-y_m, the camera optical axis). A trace generated by an
    // older/different coordinate convention can be INVERTED — that is precisely how the
    // position panel came to be drawn upside down for 25 of 84 sessions, which silently
    // reverses concentric and eccentric for anyone reviewing it. A stale artefact must
    // never be able to flip the review display: reject it and fall back to the session's
    // own signal, which is computed fresh from the marker data every time.
    if (!trace_.empty() && marker_.size() > 0) {
        double sx = 0, sy = 0, sxx = 0, syy = 0, sxy = 0;
        long n = 0;
        for (const auto& p : trace_) {
            const size_t i = static_cast<size_t>(p.frame_idx);
            if (p.frame_idx < 0 || i >= marker_.size() || !marker_.detected[i]) continue;
            const double a = p.s, b = -static_cast<double>(marker_.y_m[i]);   // vertical, up = +
        ...
        if (n >= 200) {
            ...
            const double corr = (va > 0 && vb > 0) ? cov / std::sqrt(va * vb) : 0.0;
            if (corr < 0.90) {
                spdlog::warn("SessionData: REJECTING reference trace '{}' — it disagrees "
                             "with the session's own vertical (corr={:.3f}). It is stale or "
                             "was built with a different coordinate convention; drawing it "
                             "would flip / distort the position panel. Falling back to the "
                             "session signal.", csv.string(), corr);
                trace_.clear();
                return false;
            }
        }
