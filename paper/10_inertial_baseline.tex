...
\SI{10}{\degree\per\second} (median over the corpus), because a person holding a loaded
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrr}
    ...
    \midrule
    \multicolumn{5}{l}{\emph{Per repetition, against the camera}} \\
...
\begin{table}[t]
  ...
  \begin{tabular}{lrr}
    ...
    Boundaries, displaced by $\pm 2$ frames  & 57.5   & 43.6 \\
    Boundaries, displaced by $\pm 5$ frames  & 79.0   & 66.5 \\
    Boundaries, displaced by $\pm 10$ frames & 116.6  & 117.1 \\
    Boundaries, displaced by $\pm 20$ frames & 263.2  & 237.4 \\
...
Three things follow, and the first is not what we expected.
...
\SI{56.9}{\milli\metre\per\second}). Keeping them but placing them ten frames off costs
\SI{66}{\milli\metre\per\second} (\num{50.5} to \num{116.6}). A boundary condition
imposed at the wrong instant is not a weak constraint but a wrong one: it injects a ramp
into the velocity that was not there. The practical consequence is a specification rather
than a caveat. A repetition detector that cannot place a boundary within roughly two frames
is worse than no detector at all, and one that can gains about
\SI{6}{\milli\metre\per\second} over simply high-passing.
